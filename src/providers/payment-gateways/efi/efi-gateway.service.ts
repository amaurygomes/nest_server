...
export class EfiGatewayService implements IPaymentGateway {
  ...
  constructor(
    @Inject('EFI_GATEWAY_MODULE_OPTIONS')
    private options: EfiGatewayModuleOptions,
  ) {
    const config: any = {
  ...
  async createCharge(data: CreateChargeDto): Promise<CreateChargeResponse> {
    ...
    try {
      ...
      this.logger.error(
        'EfiGateway: Error creating Pix charge:',
  ...
  async refund(data: RefundDto): Promise<RefundResponse> {
    const { transactionId, value, e2eId } = data;
    this.logger.debug(
      `EfiGateway: Refunding transaction txid ${transactionId} with value ${value}`,
    );
    try {
      const params = {
        e2eId: e2eId,
      ...
      this.logger.log(
        `EfiGateway: Refund initiated for transaction ${transactionId}.`,
      );
      ...
      this.logger.error(
        `EfiGateway: Error refunding transaction ${transactionId}:`,
        error?.response?.data || error.message,
      );
      throw error;
    }
